...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask_start_pos = random.randint(0, len_trunc_data - mask_len)
        mask_end_pos = mask_start_pos + mask_len
        # [prefix] MASK_CHAR [suffix] MASK_CHAR [masked_content] MASK_CHAR [pads]
        x = trunc_data[:mask_start_pos] + self.MASK_CHAR + trunc_data[mask_end_pos:] + self.MASK_CHAR + trunc_data[mask_start_pos:mask_end_pos] + self.MASK_CHAR
        x = x + self.PAD_CHAR*(self.block_size - len(x))
        y = x[1:] + self.PAD_CHAR
        x = torch.tensor([self.stoi[c] for c in x], dtype=torch.long)
        y = torch.tensor([self.stoi[c] for c in y], dtype=torch.long)
